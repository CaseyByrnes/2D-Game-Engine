...
void GameObject::newPlayer(int size, sf::Texture *texture, sf::Color color, sf::Vector2f startPosition)
{

    Character *temp = new Character(size, color);
...
int GameObject::newCharacter(int size, sf::Color color)
{
    Character *temp = new Character(Character(size, color));
...
void GameObject::newInputSubsystem(Character *player)
{
    inputs = new InputSubsystem(player);
}
...
int GameObject::newDeathZone(sf::Vector2f size, int posX, int posY)
{
    DeathZone *temp = new DeathZone(size, posX, posY);
