...
void FloatingPlatform::onEvent(GameEvent *e) {
    if (e->type == "FloatingPlatCollision") {
        Character *player = static_cast<Character *>(e->parameters["Character"].m_asGameItem);
        
        FloatingPlatform *platform = static_cast<FloatingPlatform *>(e->parameters["Platform"].m_asGameItem);

        float frame_delta = e->parameters["Speed"].m_asFloat;
    
        float playerHeight = player->getGlobalBounds().height;
        int playerY = player->getPosition().y;
        int playerX = player->getPosition().x;
        int floatingY = platform->getPosition().y;

        if (platform->isHorizontal()) {
            if (platform->isRight()) {
                playerX -= frame_delta;
            } else if (platform->isLeft()) {
                playerX += frame_delta;
            }
        }

        if (playerY + playerHeight > floatingY && playerY + playerHeight <= floatingY + 1 || playerY + playerHeight >= floatingY - 1 ) {
            player->setPosition(playerX, floatingY - playerHeight);
            player->allowJump();
        }
