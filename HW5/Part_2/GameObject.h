...
class GameObject
{
    public:
        GameObject();
        void newPlayer(int sizeX, int sizeY, sf::Texture *texture, sf::Color color, sf::Vector2f startPosition);
        Character *getPlayer();
        int newCharacter(int sizeX, int sizeY, sf::Color);
        ...
        void newInputSubsystem(Character *player, Timeline *mainAnchor, EventManager *eManager);
        InputSubsystem *getInputSubsystem();
        int newDeathZone(sf::Vector2f size, int posX, int posY, Timeline *anchor, EventManager *eMan);
